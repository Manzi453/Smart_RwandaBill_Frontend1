...
type ServiceType = "water" | "sanitation" | "security";
...
interface ServiceData {
    ...
    color: string; // tailwind class (used for icon)
    bgColor: string;
    stats: BaseStats;
...
const getServiceData = (service: ServiceType): ServiceData => {
    const baseData = {
        water: {
...
export const ServiceAdminDashboard: React.FC<ServiceAdminDashboardProps> = ({ onSectionChange }) => {
    const { user } = useAuth();
    const [activeTab, setActiveTab] = useState<string>("overview");
    const [searchTerm, setSearchTerm] = useState<string>("");
    const [filterStatus, setFilterStatus] = useState<string>("all");
    const [showNewBillForm, setShowNewBillForm] = useState<boolean>(false);

    // service derived from logged-in user (fallback to 'water')
    const service = (user?.service || "water") as ServiceType;
    const serviceData = useMemo(() => getServiceData(service), [service]);

    const { t } = useTranslation();

    const getServiceLabel = useCallback(() => {
        return t(`serviceAdmin.serviceTypes.${service}`);
    }, [service, t]);
        ...
        return (
            ...
            >
                ...
                <Alert className={`${serviceData.bgColor} border-2 border-current`}>
                    ...
                    <AlertDescription>
                        {t('serviceAdmin.viewDataFor', { service: getServiceLabel() })}
                    </AlertDescription>
                </Alert>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                    {[
                        {
                            label: t('serviceAdmin.metrics.totalBills'),
                            value: serviceData.stats.totalBills.toLocaleString(),
                            icon: BarChart3,
                            color: "text-blue-600",
                        },
                        {
                            label: t('serviceAdmin.metrics.paidBills'),
                            value: serviceData.stats.paidBills.toLocaleString(),
                            icon: CheckCircle,
                            color: "text-green-600",
                        },
                        {
                            label: t('serviceAdmin.metrics.pendingBills'),
                            value: serviceData.stats.pendingBills.toLocaleString(),
                            icon: Clock,
                            color: "text-yellow-600",
                        },
                        {
                            label: t('serviceAdmin.metrics.overdueBills'),
                ...
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                    <Card>
                        <CardHeader>
                            <CardTitle>{t('serviceAdmin.metrics.revenueTrend')}</CardTitle>
                        </CardHeader>
                        <CardContent>
                            <ResponsiveContainer width="100%" height={300}>
                                <LineChart data={serviceData.monthlyTrend}>
                                    ...
                                    <Legend />
                                    {/* choose colors by service */}
                                    <Line
                                        type="monotone"
                                        dataKey="revenue"
                                        stroke={service === "water" ? "#0088FE" : service === "sanitation" ? "#FF8C42" : "#00C49F"}
                                        strokeWidth={2}
                                        name="Revenue"
                                    />
                                </LineChart>
                    ...
                    <Card>
                        <CardHeader>
                            <CardTitle>{t('serviceAdmin.metrics.districtRevenue')}</CardTitle>
                        </CardHeader>
                        <CardContent>
                            <ResponsiveContainer width="100%" height={300}>
                                <PieChart>
                                    <Pie
                                        ...
                                        cy="50%"
                                        labelLine={false}
                                        label={(entry: DistrictBreakdown) => `${entry.district} ${entry.percentage}%`}
                                        outerRadius={80}
                                        fill="#8884d8"
                                        dataKey="revenue"
                                    >
    ...
    }, [serviceData, getServiceLabel, service]);
    ...
    const renderTransactionsTab = useCallback(() => {
        return (
            ...
            >
                <Card>
                    <CardHeader>
                        <CardTitle>{t('serviceAdmin.transactions.recentTransactions')}</CardTitle>
                    </CardHeader>
                    <CardContent>
                        <div className="overflow-x-auto">
                            <table className="w-full">
                                ...
                                <tr>
                                    <th className="px-4 py-3 text-left text-xs font-medium text-muted-foreground">{t('serviceAdmin.transactions.user')}</th>
                                    <th className="px-4 py-3 text-left text-xs font-medium text-muted-foreground">{t('serviceAdmin.transactions.amount')}</th>
                                    <th className="px-4 py-3 text-left text-xs font-medium text-muted-foreground">{t('serviceAdmin.transactions.status')}</th>
                                    <th className="px-4 py-3 text-left text-xs font-medium text-muted-foreground">{t('serviceAdmin.transactions.date')}</th>
                                    <th className="px-4 py-3 text-left text-xs font-medium text-muted-foreground">{t('serviceAdmin.transactions.action')}</th>
                                </tr>
                                </thead>
                        ...
                        >
                          {transaction.status.charAt(0).toUpperCase() + transaction.status.slice(1)}
                                        ...
                                        <td className="px-4 py-3 text-sm">
                                            <Button variant="outline" size="sm">{t('serviceAdmin.transactions.view')}</Button>
                                        </td>
                                    </tr>
                                ))}
                                </tbody>
    ...
    }, [serviceData.recentTransactions]);
        ...
        return (
            ...
            >
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <Card>
                        <CardHeader>
                            <CardTitle>{t('serviceAdmin.reports.generate')}</CardTitle>
                        </CardHeader>
    ...
    }, [getServiceLabel, serviceData.stats]);

    /* -------------------------
       Users / Bills filtered lists
    ------------------------- */

    const filteredUsers = useMemo(() => {
        const s = searchTerm.toLowerCase();
        return serviceData.users.filter((u) => {
            const matchesSearch = u.name.toLowerCase().includes(s) || u.email.toLowerCase().includes(s);
            const matchesStatus = filterStatus === "all" || u.status === (filterStatus as UserStatus);
            return matchesSearch && matchesStatus;
        });
    }, [serviceData.users, searchTerm, filterStatus]);

    const filteredBills = useMemo(() => {
        const s = searchTerm.toLowerCase();
        return serviceData.bills.filter((b) => {
            const matchesSearch = b.userName.toLowerCase().includes(s);
            const matchesStatus = filterStatus === "all" || b.status === (filterStatus as BillStatus);
            return matchesSearch && matchesStatus;
        });
    }, [serviceData.bills, searchTerm, filterStatus]);
                ...
                <Card>
                    <CardHeader>
                        <CardTitle className="flex items-center justify-between">
                            <span className="flex items-center gap-2"><Users className="h-5 w-5" /> Service Users</span>
                                    ...
                                    <tr key={u.id} className="hover:bg-muted/30">
                                        ...
                                        <td className="px-4 py-3 text-sm">
                                            <div className="flex items-center gap-1">
                                                <MapPin className="h-3 w-3 text-muted-foreground" />{u.district}
                                            </div>
                                        </td>
                                        <td className="px-4 py-3 text-sm">
                                            <div className="text-xs">
                                                <div className="font-semibold">{u.paidBills}/{u.totalBills}</div>
                        ...
                        <span className={`inline-flex px-2 py-1 text-xs font-semibold rounded-full ${
                          ...
                          {u.status.charAt(0).toUpperCase() + u.status.slice(1)}
                                        ...
                                        <td className="px-4 py-3 text-sm flex gap-2">
                                            ...
                                            <Button variant="outline" size="sm" className="h-8 w-8 p-0"><Mail className="h-4 w-4" /></Button>
                                        </td>
                                    </tr>
                                ))}
                                </tbody>
    ...
    }, [filteredUsers, searchTerm, filterStatus]);

    /* -------------------------
       Bills Tab (UI + Create form)
    ------------------------- */
    const renderBillsTab = useCallback(() => {
        return (
            <motion.div
                key="bills"
                initial={{ opacity: 0, y: 20 }}
            ...
            >
                <div className="flex justify-between items-center">
                    <h2 className="text-xl font-bold flex items-center gap-2"><FileText className="h-5 w-5" /> Bill Management</h2>
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                <div>
                                    ...
                                    <select className="w-full mt-2 px-3 py-2 border rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500">
                                        ...
                                        {serviceData.users.map((u) => <option key={u.id} value={u.id}>{u.name}</option>)}
                                    </select>
                                </div>
                                <div>
                                    <label className="text-sm font-medium">Amount (RWF)</label>
                                    <input type="number" placeholder={serviceData.stats.averageBillAmount.toString()} className="w-full mt-2 px-3 py-2 border rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500" />
                                </div>
                                <div>
                                    <label className="text-sm font-medium">Due Date</label>
                                    <input type="date" className="w-full mt-2 px-3 py-2 border rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500" />
                                </div>
                                <div>
                                    <label className="text-sm font-medium">Description</label>
                                    <input type="text" placeholder="Bill description" className="w-full mt-2 px-3 py-2 border rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500" />
                                </div>
                            </div>
                            <div className="flex gap-2">
                                <Button className="flex-1">Create Bill</Button>
                                <Button variant="outline" className="flex-1" onClick={() => setShowNewBillForm(false)}>Cancel</Button>
                            </div>
                        </CardContent>
                    </Card>
                )}
                ...
                <Card>
                    <CardHeader>
                        <CardTitle className="flex items-center justify-between">
                            <span>All Bills</span>
                            <span className="text-sm font-normal text-muted-foreground">{filteredBills.length} bills</span>
                        </CardTitle>
                    </CardHeader>
                    <CardContent className="space-y-4">
                        <div className="flex gap-4 flex-col md:flex-row">
                            <div className="flex-1 relative">
                                <Search className="absolute left-3 top-3 h-4 w-4 text-muted-foreground" />
                                <input type="text" placeholder="Search by user name..." value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} className="w-full pl-10 pr-4 py-2 border rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500" />
                            </div>
                            ...
                            <table className="w-full">
                                ...
                                {filteredBills.map((bill) => (
                                    <tr key={bill.id} className="hover:bg-muted/30">
                                        ...
                                        <td className="px-4 py-3 text-sm">
                        <span className={`inline-flex px-2 py-1 text-xs font-semibold rounded-full ${bill.status === "paid" ? "bg-green-100 text-green-800" : bill.status === "pending" ? "bg-yellow-100 text-yellow-800" : "bg-red-100 text-red-800"}`}>
                          {bill.status.charAt(0).toUpperCase() + bill.status.slice(1)}
                                        ...
                                        <td className="px-4 py-3 text-sm flex gap-2">
                                            <Button variant="outline" size="sm" className="h-8 w-8 p-0"><Eye className="h-4 w-4" /></Button>
                                            <Button variant="outline" size="sm" className="h-8 w-8 p-0"><Edit2 className="h-4 w-4" /></Button>
                                            <Button variant="outline" size="sm" className="h-8 w-8 p-0 text-red-600 hover:text-red-700"><Trash className="h-4 w-4" /></Button>
                                        </td>
                                    </tr>
                                ))}
                                </tbody>
    ...
    }, [filteredBills, searchTerm, filterStatus, showNewBillForm, serviceData.bills]);
        ...
        <div className="space-y-6">
            ...
            <div className="flex items-center justify-between">
                <div className="flex items-center gap-3">
                    <div className={`p-3 rounded-lg ${serviceData.bgColor}`}>{serviceData.icon}</div>
                    <div>
                ...
                {[
                    ...
                    return (
                        <button
                            ...
                            className={`px-4 py-2 flex items-center gap-2 border-b-2 transition-colors whitespace-nowrap ${activeTab === tab.id ? "border-blue-600 text-blue-600" : "border-transparent text-muted-foreground hover:text-foreground"}`}
                        >
